...
refTable* createRefTable(uint32_t size) {
    ...
    if (dict == NULL) raiseException("dictError", "Failed to allocate memory for refTable.");
    dict->tableSize = size;
    dict->numEntries = 0;
...
void refTableInsert(refTable* dict, char* key, uint16_t value) {
    ...
    if (entry == NULL) raiseException("dictError", "Failed to allocate memory for reference table entry");
    entry->key = key;
    entry->value = value;
    ...
    if ((float)dict->numEntries / (float) dict->tableSize > 0.75) {
        if (dict->tableSize >= (UINT32_MAX/2)) raiseException("dictError", "Reference table size exceeds maximum size during reallocation");
...
uint16_t refTableGet(refTable* dict, char* key) {
    ...
    raiseException("dictError", "Key not found in reference table");
    return 0;
}
